...
export const NAV_STYRK08: Record<string, NavOccupationMatch> = {
  ...
  // в 64%, из заполнивших Arbeidsspråk 54% допускают английский. Ресепшн
  // (4224/4226) по-прежнему не берётся: стойка регистрации — это разговор с
  // гостем на языке гостя, не готовка на кухне.
  ...
  // --- Официанты (23.09.2026, реальный запрос соискательницы). Прежде не
  // брались тем же решением, что и кухня, — предположение "работа с гостями
  // предполагает язык" оказалось неверным при проверке: 24/24 объявления в
  // наших кластерах дали чистый код 5131 без подмешивания к другим ролям
  // (сравни с 8332, где такси подмешивалось к грузоперевозкам), норвежский
  // явно требуют 4.2% (1 из 24), английский допускают 33.3%, жильё и дорога
  // — 0%, тот же профиль, что уже принят для kokk/köksbiträde. Ловушек уровня
  // круизов/частных мероприятий не нашлось — везде наземные рестораны и залы.
  "5131": { term: "servitör", legalMinimumSector: "hospitality" },
